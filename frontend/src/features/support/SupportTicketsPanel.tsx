...
export function SupportTicketsPanel({ mode }: { mode: Mode }) {
      ...
      <ScrollView contentContainerStyle={styles.page}>
        ...
        <View>
          ...
          {(detail.comments || []).length === 0 ? (
            <SofEmptyState title="Nenhum comentário ainda" />
          ) : (
  ...
  return (
    <ScrollView contentContainerStyle={styles.page}>
      <SofPageHeader
        title={`Suporte${mode === 'account' && has('supportPriority') ? ' · Prioritário' : ''}`}
        subtitle={
          mode === 'account'
            ? has('supportPriority')
      ...
      {loading ? (
        ...
        <SofCard padded={false}>
          <SofEmptyState title="Nenhum ticket ainda" />
        </SofCard>
      ) : (
        <SofCard padded={false}>
          {tickets.map((t) => (
            <SofListRow
              key={t.id}
              title={t.title}
              meta={`${t.createdByName} · ${formatWhen(t.updatedAt)} · ${t.commentCount} comentário(s)`}
