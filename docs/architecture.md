...
| WhatsApp (Uazapi, default) | Bot off; simulador em `/simulador` | `WHATSAPP_BASE_URL` + admin token (multi-conta) ou token de instância; QR/código na Conta; menus via `/send/menu`; áudio transcrito via `/message/download` + `OPENAI_API_KEY`; se o telefone for de um `Employee`, FSM em `whatsapp-employee-bot.service.ts` (agenda / marcar / evento / concluir / cancelar / falar com estabelecimento + NLU próprio) |
...
| `/(dashboard)/simulador` | Simulador WhatsApp (`noindex`; fora das tabs) |
